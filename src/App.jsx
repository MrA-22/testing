...
export default function LiveLoveRoomWithPhotobooth() {
  ...
  // Canvas & MediaPipe Refs untuk Virtual Background Live Preview
  ...
  // Load MediaPipe script dengan Inisialisasi
  useEffect(() => {
  ...
  // Loop Render Live Preview dengan Virtual Background yang Benar
  useEffect(() => {
    if (boothStep === 'preview' && cameraActive) {
  ...
  // Perbaikan Logika Komposisi Canvas (Badan di Depan, Background di Belakang)
  const onMediaPipeResults = (results) => {
    ...
    // 1. Gambar video orang (mirror) ke canvas terlebih dahulu
    ctx.save();
    ...
    // 2. Potong canvas agar hanya menyisakan bentuk tubuh orangnya saja (Masking)
    ctx.globalCompositeOperation = 'destination-in';
    ctx.drawImage(results.segmentationMask, 0, 0, canvas.width, canvas.height);

    // 3. Gambar background tema di belakang area yang kosong (di belakang badan)
  ...
  const runDualCaptureStep = async () => {
    ...
    const timer = setInterval(async () => {
      ...
      if (count > 0) {
        ...
        // Ambil hasil gambar dari canvas preview (yang sudah ber-virtual background rapi)
        if (previewCanvasRef.current) {
          ...
          ctx.clip();
          ctx.drawImage(previewCanvasRef.current, 40, 60, 340, 460);
          ctx.restore();
        }
                ...
                {boothStep === 'select-layout' && (
                  <div className="space-y-3 w-full max-w-xs text-left my-auto">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-stone-500">Pilih tema virtual background kamera kamu!</p>
                    </div>
                  ...
                  <div className="space-y-3 w-full text-center my-auto">
                    ...
                    <div className="bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 border-2 border-rose-200 p-3 rounded-3xl shadow-lg max-w-[380px] mx-auto space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2.5">
                        {/* Kamera Kamu (Virtual Background Bersih) */}
                ...
                {boothStep === 'ready' && finalStripUrl && (
                  <div className="space-y-2 w-full flex flex-col items-center my-auto pt-1">
                    ...
                    <div className="flex gap-2 w-full max-w-[280px]">
                      <a href={finalStripUrl} download={`DualPhotobooth_${myName}_${partnerName}.png`} className="flex-1 py-2.5 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold rounded-xl shadow-md text-xs text-center block cursor-pointer hover:scale-105 transition">📥 Download (PNG)</a>
